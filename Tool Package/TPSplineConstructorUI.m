...
classdef TPSplineConstructorUI < handle
    properties
        NodeTimes;
    ...
    methods
        function obj = TPSplineConstructorUI()
        end
        ...
        function GenerateSplines(obj, NodeTimes, ZeroOrderDerivativeNodes, FirstOrderDerivativeNodes, SecondOrderDerivativeNodes)
            ...
            for i = 1:obj.NumberOfSplines
                ...
                %Generating splines
                obj.SplineArray.ZeroOrderDerivative(i) = TPSplineConstructor(Segments, Times_for_this_spline);
        ...
        %This function plots the splines
        function figure_handle = Plot(obj, Type, LineWidth, dt, figure_handle)
            
            t0 = min(obj.NodeTimes);
            Time = max(obj.NodeTimes) - t0;
            
            if nargin < 5
                figure_handle = [];
            end
            
            %If dt wasn't specified - make it so there are only 100 points
            %on each graph.
            if nargin < 4
                dt = [];
            end
            ...
            Count = floor(Time/dt) + 1;
            DataArray = zeros(Count, obj.NumberOfSplines);
            ...
            switch Type
                ...
                case {'V', 'v'}
                    for i = 1:Count
                        t = (i - 1)*dt + t0;
                        DataArray(i, :) = obj.EvaluateV(t)';
                        TimeArray(i) = t;
                        y_axis_label = '$$\frac{dq_i}{dt}$$';
                    end
                case {'A', 'a'}
                    for i = 1:Count
                        t = (i - 1)*dt + t0;
                        DataArray(i, :) = obj.EvaluateA(t)';
                        TimeArray(i) = t;
                        y_axis_label = '$$\frac{d^2 q_i}{dt^2}$$';
                    end
                otherwise
                    warrning('Invalid plot request')
            end
            ...
            BP.Plot(TimeArray, DataArray, 't, s', y_axis_label, figure_handle);
            
            switch Type
                case {'Q', 'q'}
                    title('Splines')
                case {'V', 'v'}
                    title('First derivatives of the splines')
                case {'A', 'a'}
                    title('Second derivatives of the splines')
                otherwise
            end
        end
